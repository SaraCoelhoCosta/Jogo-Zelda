...
def play(win, second_win, size_win, hyrule):
    ...
    print(f'Início para dugeon 1: {len(path)}')
    ...
    for i, dungeon in enumerate(order_way): # Percorrendo as dungers
        ...
        distance = distance + len(path)
        print(f'Percorrendo dugeon {i+1}: {len(path)}')
        ...
        distance = distance + len(path)
        print(f'Retornando para início da dugeon {i+1}: {len(path)}')
        ...
        if i < len(order_way) - 1:
            ...
            distance = distance + len(path)
            print(f'Percorrendo hyrule para dungeon {i+1}: {len(path)}')
        else:
            ...
            distance = distance + len(path)
            print(f'Percorrendo da dungeon {i+1} para o fim: {len(path)}')
...
def best_way(win, size_win, hyrule):
        ...
        for ways_list in perm_list:
            ...
            for i, dungeon in enumerate(ways_list):
                ...
                distance = len(path)
                distance_way = distance_way + distance
            ...
            all_paths.append((points_dungeon, distance_way)) # Guarda posição da dungeon e caminho percorrido
            # print(f'Interação: {points_dungeon, distance_way}')
