...
def play(win, second_win, size_win, hyrule):
    ...
    path = algorithm(win, lambda:draw_points(win, size_win, hyrule.size, hyrule.points), hyrule.points, order_way[0].start_hyrule, hyrule.start) # Início para dungeon
    ...
    for i, dungeon in enumerate(order_way): # Percorrendo as dungers
        ...
        path = algorithm(win, lambda:draw_points(second_win, size_win, dungeon.size, dungeon.points), dungeon.points, dungeon.end, dungeon.start) # Percorre dungeon até pingente
        distance = calculate_path(path)
        distance_total = distance_total + distance
        print(f'Percorrendo dungeon {i+1}: {distance}')
        
        path = algorithm(win, lambda:draw_points(second_win, size_win, dungeon.size, dungeon.points), dungeon.points, dungeon.start, dungeon.end) # Retorna para entrada da dungeon
        distance = calculate_path(path)
        distance_total = distance_total + distance
        print(f'Retornando para início da dungeon {i+1}: {distance}')
                    
        if i < len(order_way) - 1:
            path = algorithm(win, lambda:draw_points(win, size_win, hyrule.size, hyrule.points), hyrule.points, order_way[i + 1].start_hyrule, dungeon.start_hyrule) # Percorre o Hyrule até a próxima dungeon
            distance = calculate_path(path)
            distance_total = distance_total + distance
            print(f'Percorrendo hyrule para dungeon {i+2}: {distance}')
        else:
            path = algorithm(win, lambda:draw_points(win, size_win, hyrule.size, hyrule.points), hyrule.points, end, dungeon.start_hyrule) # Percorre o Hyrule para o final
            distance = calculate_path(path)
            distance_total = distance_total + distance
...
def best_way(win, size_win, hyrule):
        ...
        for ways_list in perm_list:
            ...
            for i, dungeon in enumerate(ways_list):
                path = algorithm(win, lambda: draw_points(win, size_win, hyrule.size, hyrule.points), hyrule.points,
